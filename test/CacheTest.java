...
public class CacheTest {
	 ...
	 private class StringLoader implements DataProvider<Integer, String> {
		public StringLoader() {
		}
	...
	@Before
	public void setUp() {
		provider = new StringLoader();
		cache = new LRUCache<Integer, String>(provider, capacity);
	}
	...
	public void leastRecentlyUsedIsCorrect() {
		...
		// calls key 1, which should no longer be in the cache since it was the
		// one called longest ago, and so there should be another miss
		cache.get(1);
		assertEquals(cache.getNumMisses(), capacity + 2);

		// the call of key 1 should now be added to the hash and the one used
		// the longest ago, key 2, should be removed
		cache.get(2);
		assertEquals(cache.getNumMisses(), capacity + 3);

		// calling something still in the cache (the last element put in by the
		// for loop) should not lead to a miss
